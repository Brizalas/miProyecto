...
Arquitectura actual durante la transición al Service:
...
En este momento el `Controller` conserva temporalmente el `Repository` porque todavía hay operaciones que no se han migrado al `Service`. El listado ya pasa por `AlumnoService`.
...
AlumnoService.java
→ centraliza operaciones y lógica relacionada con los alumnos

HomeController.java
→ recibe peticiones, prepara las vistas y delega operaciones
...
# 8. Primer `AlumnoService`

Archivo:

```text
src/main/java/com/cristian/gestoralumnos/service/AlumnoService.java
```
...
Indica a Spring que esta clase forma parte de la lógica de servicio de la aplicación.

Spring crea y administra automáticamente un objeto de `AlumnoService`, igual que hace con clases anotadas con:

```java
@Controller
```

## Inyección del Repository en el Service

```java
private final AlumnoRepository alumnoRepository;
```

El Service necesita el Repository para acceder a los datos.
...
En esta primera fase el método parece una simple repetición, pero abre la puerta a centralizar reglas, comprobaciones y operaciones más complejas.

---

# 9. Controller completo en transición
...
# 10. Conexión entre formulario y objeto
...
# 16. Gestión de un ID inexistente al editar

La interfaz normal genera el botón `Editar` a partir de alumnos que ya existen.

Sin embargo, la ruta también puede escribirse manualmente:

```text
http://localhost:8080/editar-alumne?id=999
```

También puede ocurrir que un alumno se elimine después de cargar la página y antes de pulsar el botón.

Por eso el Controller no debe confiar ciegamente en que el ID recibido siempre existe.
...
Flujo con un ID inexistente:

```text
GET /editar-alumne?id=999
↓
findById(999)
↓
orElse(null)
...
El `return` termina el método inmediatamente, por lo que nunca se envía esto a Thymeleaf:
...
ID inexistente en `GET /editar-alumne`:
...
# 18. Por qué Update busca de nuevo el alumno
...
# 22. Errores importantes encontrados
...
# 25. Estado exacto para retomar el proyecto

```text
Entidad JPA                          ✅
ID autogenerado                     ✅
Repository                          ✅
Primer AlumnoService creado          ✅
Listado migrado al Service           ✅
H2                                  ✅
Create                              ✅
Read                                ✅
Update                              ✅
...
data-th-object                      ✅
data-th-field                       ✅
ID inexistente en editar controlado ✅
```

Estado actual de la migración:

```text
HomeController usa AlumnoService
para listar alumnos.

HomeController todavía usa
AlumnoRepository directamente
para guardar, buscar y eliminar.
```
...
```text
1. Gestionar ID inexistente en editar        ✅
2. Crear AlumnoService                       ✅
3. Migrar el listado al Service              ✅
```

Pendiente:

```text
1. Mover save() al Service.
2. Mover findById() al Service.
3. Mover deleteById() al Service.
4. Eliminar AlumnoRepository del Controller.
5. Decidir una respuesta más informativa
   para los IDs inexistentes.
```
...
No es obligatorio que toda entidad tenga siempre exactamente un Repository y un Service, aunque es frecuente:
...
Un Service también puede coordinar varios Repositories cuando una operación afecta a distintas entidades.
...
Service
→ centraliza operaciones y reglas

Controller
→ dirige la petición y delega
...
ID inexistente en editar controlado
↓
AlumnoService creado
↓
listarAlumnos() ya pasa por el Service
↓
Siguiente paso:
mover save() al AlumnoService
```
